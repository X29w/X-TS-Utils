...
import { LoadingCounter, useLoadingCounter } from './loading-counter';

export class AxiosKit implements Base.AxiosKit {
  // AxiosKit 一些配置(启用查询参数过滤、启用请求日志等等)
  ...
  // 状态码拦截器
  __statusInterceptor: Base.IStatusInterceptorCallback = () => {};
  // Loading 计数控制器实例
  __loadingInstance: Util.LoadingCounter | null = null;
   ...
   * @returns { AxiosKit }                             当前 AxiosKit 实例
   */
  create(
    ...
    this.axiosInstance = axios.create(
      Object.assign({}, useAxiosDefaultRequestConfig(qs), config),
    );
    return this;
  }
    ...
    const interceptorId = this.axiosInstance.interceptors.request.use(
      (config) => {
        ...
        if (__isFunction(beforeRequestHandler)) {
          config = <Axios.InternalAxiosRequestConfig<unknown>>(
            (beforeRequestHandler?.(config) || config)
          );
        }
        ...
        if (this.config.enableLog) {
          ...
          console.groupCollapsed(
            `%crequest - success  ${url}`,
            `color:${this.config.successFontColor}`,
          );
      ...
      (error) => {
        ...
        if (this.config.enableLog) {
          console.groupCollapsed(
            `%crequest - error  ${error?.url}`,
            `color:${this.config.errorFontColor}`,
          );
          console.error(error);
          console.groupEnd();
        }
  ...
  destroyRequestInterceptors(interceptorId: number): void {
    requestInstanceErrorCheck(this.axiosInstance);
    const removeIndex = this.requestInterceptorsIds.findIndex(
      (id) => id === interceptorId,
    );
    if (removeIndex === -1) {
      return;
    ...
    const interceptorId = this.axiosInstance.interceptors.response.use(
      (response) => {
        ...
        if (this.config.enableLog) {
          ...
          console.groupCollapsed(
            `%cresponse - success  ${response.config.url}`,
            `color:${this.config.successFontColor}`,
          );
          console.log('data ', data);
      ...
      (error) => {
        ...
        if (this.config.enableLog) {
          ...
          console.groupCollapsed(
            `%cresponse - error  ${error.config.url}`,
            `color:${this.config.errorFontColor}`,
          );
  ...
  destroyResponseInterceptors(interceptorId: number): void {
    requestInstanceErrorCheck(this.axiosInstance);
    const removeIndex = this.responseInterceptorsIds.findIndex(
      (id) => id === interceptorId,
    );
    if (removeIndex === -1) {
      return;
   ...
   * @returns { AxiosKit }                                                当前 AxiosKit 实例
   */
  useStatusInterceptors(
    callback: Base.IStatusInterceptorCallback,
  ): Base.AxiosKit {
  ...
  request<T, R = unknown>(config: Base.IRequestConfig<T>): Promise<R> {
    ...
    return new Promise((resolve, reject) => {
      const requestOptions = <Axios.AxiosRequestConfig>{
        ...
        params:
          !disableDataAutoDifferentiate &&
          method === EasyAxiosRequestQueryParamsMethod
            ? data
            : params,
        data:
          !disableDataAutoDifferentiate &&
      ...
      this.axiosInstance
        ...
        .then((response) => {
          if (__isFunction(this.__statusInterceptor)) {
            return this.__statusInterceptor<R>({
              response,
              disableToast,
              resolve,
              reject,
        ...
        .finally(() => {
          if (
            !disableLoading &&
            this.__loadingInstance instanceof LoadingCounter
          ) {
   ...
   * @returns { Promise<R> }
   */
  streaming<T, R = unknown>(
    config: Base.IStreamingConfig<T>,
  ): Promise<R | Base.IStreamingDownloadResponse> {
    requestInstanceErrorCheck(this.axiosInstance);
    const [errorField, errorInfo] = requestBaseErrorCheck<T>(config).split(':');
    if (errorField !== '') {
      throw new Error(`request(...) ${errorField} 字段错误, ${errorInfo}`);
    ...
    let { method, data = {}, headers = {}, responseType = 'json' } = config;
    const {
      interfacePath,
      mode = 'Default',
      params = {},
    ...
    if (mode === 'Upload') {
      ...
      headers = Object.assign(
        {},
        { 'Content-Type': EasyAxiosContentTypeOptions.FormData },
        headers,
      );
    }
          ...
          if (mode === 'Download') {
            ...
            if (__isFunction(customDownloadResponse)) {
              return resolve(
                <R>(
                  customDownloadResponse?.(<
                    { headers: Axios.AxiosResponseHeaders; data: Blob }
                  >{ headers, data })
                ),
              );
            }
            ...
            // 2) 默认响应体构建
            const streamConfig = parseResponseHeaderQueryParameters(
