...
export const __isFunction = (value: unknown): boolean =>
...
export const __isEmptyObject = function (p: unknown): boolean {
  if (__isObject(p)) {
    for (const k in p as object) {
      if ((p as object).hasOwnProperty(k)) {
        return false;
      }
    }
    return true;
 ...
 */
export const requestParamsFilter = (
  data: Record<string, any>,
  eachCallback?:  (key: string, value: any) => void,
): void => {
...
export const streamingCreateFormData = <Util.IStreamingCreateFormData>(<T>({
  ...
  // 添加参数
  requestParamsFilter(data as Record<string, any>, (key: string, value: string) =>
  ...
  if (fileCount) {
    if (__isFunction(customSequence)) {
      ...
      files.forEach((file, index) =>
        form.append(`${fileField}${enabled ? `[${index}]` : ''}`, file),
      );
    }
  }
 ...
 */
export const parseResponseHeaderQueryParameters = <
  Util.IParseResponseHeaderQueryParameters
>((resource) => {
  const result = {};
  if (typeof resource === 'string' && resource.length) {
    const decodedResource = decodeURIComponent(resource);
    const keyValueStrings = decodedResource.split('&');
 ...
 */
export const requestInstanceErrorCheck = (
  axiosInstance: Axios.AxiosInstance | null,
) => {
  if (axiosInstance === null) {
    throw new Error(
      `request(...) 未实例化 axios 实例, 使用 AxiosKit 实例化后需要先调用 create 创建 axaios 实例`,
    );
  }
  return false;
};
 ...
 */
export const requestBaseErrorCheck = <T>(
  config: Base.IRequestConfig<T>,
): string => {
